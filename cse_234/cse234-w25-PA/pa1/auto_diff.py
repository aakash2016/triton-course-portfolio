...
class DivOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        return [
            ...
            mul_by_const(output_grad, -1) * node.inputs[0] / power(node.inputs[1], 2)
        ]
...
class MatMulOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        grad_A = matmul(output_grad, transpose(node.inputs[1], dim0=-1, dim1=-2))
        grad_B = matmul(transpose(node.inputs[0], dim0=-2, dim1=-1), output_grad)
...
class SoftmaxOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # preventing overflow using https://pytorch.org/docs/stable/generated/torch.logsumexp.html
        return torch.exp(input_values[0] - torch.logsumexp(input_values[0], dim=node.dim, keepdim=True))

    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        """Given gradient of softmax node, return partial adjoint to input."""
        softmax_vals = softmax(node.inputs[0], dim=node.dim)
        s_grad = softmax_vals * output_grad
        return [sub(s_grad, softmax_vals * sum_op(s_grad, dim=node.dim, keepdim=True))]
...
class LayerNormOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        mean_val = input_values[0].mean(dim=tuple(range(-len(node.normalized_shape), 0)), keepdim=True)
        var_val = input_values[0].var(dim=tuple(range(-len(node.normalized_shape), 0)), keepdim=True, unbiased=False)
        return (input_values[0] - mean_val) / torch.sqrt(node.eps + var_val)
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        size = sum_op(ones_like(node.inputs[0]), dim=(dim,), keepdim=True)

        mean_val = mean(node.inputs[0], dim=dim, keepdim=True)
        var_val = variance(node.inputs[0], dim=dim, keepdim=True)
        c = power(sqrt(add_by_const(var_val, node.eps)), -1)
        normalised_mean = (node.inputs[0] - mean_val) / mul_by_const(power(var_val + node.eps, 1.5), 2)
        ...
        return [(output_grad * c) - grad_mu - (grad_var * 2 * (node.inputs[0] - mean_val) / size)]
...
class ReLUOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        return [output_grad * greater_than_const(node.inputs[0], 0)]
...
class PowerOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        return [output_grad * mul_by_const(power(node.inputs[0], node.exponent-1), node.exponent)]
...
class MeanOp(Op):
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        if node.keepdim:
            return [
                expand_as(div(output_grad, size), node.inputs[0])
            ]
        else:
            return [
                div(expand_as_3d(output_grad, node.inputs[0]), size)
            ]
...
class VarianceOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        assert len(input_values) == 1
        return input_values[0].var(dim=node.dim, keepdim=node.keepdim, unbiased=False)
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        if keepdim:
            mean_val = mean(node.inputs[0], dim, keepdim)
            diff = sub(node.inputs[0], mean_val)
            grad = output_grad * 2 * div(diff, size)
            return [grad]
        else:
            mean_val = expand_as_3d(mean(node.inputs[0], dim, keepdim), node.inputs[0])
            diff = sub(node.inputs[0], mean_val)
            grad = expand_as_3d(output_grad, node.inputs[0]) * 2 * div(diff, size)
            return [grad]
...
greater_than_const = GreaterThanConstOp()
...
class Evaluator:
    ...
    def run(self, input_values: Dict[Node, torch.Tensor]) -> List[torch.Tensor]:
        ...
        # compute values for each node in topological order
        for node in sorted_nodes:
            if node not in computed_values:
                input_values_list = [computed_values[input_node] for input_node in node.inputs]
                computed_values[node] = node.op.compute(node, input_values_list)
...
def gradients(output_node: Node, nodes: List[Node]) -> List[Node]:
    ...
    for node in topological_sort([output_node])[::-1]:
        current_grad = node_to_grad[node]
        if not node.inputs:
            continue

        input_grads = node.op.gradient(node, current_grad)

        # propagate the gradients to the input nodes
        for input_node, grad in zip(node.inputs, input_grads):
            if input_node not in node_to_grad:
                node_to_grad[input_node] = grad
            else:
                node_to_grad[input_node] += grad  # accumulate gradients if node appears multiple times

    return [node_to_grad[ip] for ip in nodes]
